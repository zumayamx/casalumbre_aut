...
EXEC ObtenerLiquidosBase @ID_Liquido = 16;
SELECT * FROM Liquidos;

-- Eliminar el procedimiento almacenado si ya existe
IF OBJECT_ID('ObtenerLiquidosBaseW', 'P') IS NOT NULL
DROP PROCEDURE ObtenerLiquidosBaseW;
GO

CREATE PROCEDURE ObtenerLiquidosBaseW
    @ID_Liquido INT
AS
BEGIN
    WITH CTE_Liquidos AS (
        ...
        SELECT 
            ...
            CAST(cte.Rastro + '->' + CAST(l.ID_Liquido AS VARCHAR(MAX)) AS VARCHAR(MAX)) AS Rastro,
            CASE 
                WHEN l.ID_Transaccion = cte.ID_Transaccion THEN cte.Nivel 
                ELSE cte.Nivel + 1 
            END AS Nivel -- Mantener el nivel si es la misma transacción, incrementar si es diferente
        FROM Liquidos l
        INNER JOIN CTE_Liquidos cte ON l.ID_Liquido IN (cte.ID_Liquido_A, cte.ID_Liquido_B)
        WHERE CHARINDEX('->' + CAST(l.ID_Liquido AS VARCHAR(MAX)) + '->', '->' + cte.Rastro + '->') = 0 -- Evitar ciclos y duplicados
    )
